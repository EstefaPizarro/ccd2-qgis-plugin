...
// EXPERIMENTAL variant: dropdown to switch between NDVI/EVI/NBR/NBI (chart + map color
// composite), instead of the original's fixed NDVI-only view. See ccd_dashboard_app.js for
// the version actually wired to the plugin's "GEE App" button - this one is a standalone
// copy to try out multi-index support without touching that one.
//
// Not run from Python - paste this into code.earthengine.google.com, run it once to
// preview, then Apps > Publish new App (as a *separate* app from the original if you want
// to keep both).
//
// Band-prep/index formulas below intentionally mirror CCD_Plugin/core/gee_data_sentinel.py
// (NDVI/NBR/EVI - same scale factors, same band names). NBI (here: NDBI, the built-up
// index) has no equivalent in that Python file - it's specific to this experiment.
//
...
// layer below (map.layers().reset() clears it each time a layer changes).
...
function chartOptions(sensorLabel) {
  return {title: sensorLabel + ' ' + selectedIndex + ' (drag to zoom, right-click to reset - click a point to load that image)',
...
// factory) since there's only ever exactly one chart per sensor, no need for more structure.
...
// Kept as module-level vars (not returned) so onIndexChange() can swap widget index 1
// in-place when the chart is rebuilt for a new index.
...
var s2ChartPanel = chartPanel(s2ResetZoomBtn, s2Chart);
var l8ChartPanel = chartPanel(l8ResetZoomBtn, l8Chart);
