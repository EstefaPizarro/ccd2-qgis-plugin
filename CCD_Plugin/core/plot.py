...
def generate_plot(id, ccdc_result_info, timeseries, date_range, dataset, band_or_index_to_plot):
    ...
    if ccdc_result_info["tBreak"]:
        ...
        for seg in range(nsegments):
            ...
            # bug fix (CLAUDE.md 4.3): must alternate cos/sin per harmonic pair - the
            # original repeated cos() for coefs[3]/[5]/[7], which are the sin terms.
            pred = [
                coefs[0]
                + coefs[1] * t
                + coefs[2] * np.cos(t * 1 * 2 * np.pi / (365.25 * 24 * 60 * 60 * 1000))
                + coefs[3] * np.sin(t * 1 * 2 * np.pi / (365.25 * 24 * 60 * 60 * 1000))
                + coefs[4] * np.cos(t * 2 * 2 * np.pi / (365.25 * 24 * 60 * 60 * 1000))
                + coefs[5] * np.sin(t * 2 * 2 * np.pi / (365.25 * 24 * 60 * 60 * 1000))
                + coefs[6] * np.cos(t * 3 * 2 * np.pi / (365.25 * 24 * 60 * 60 * 1000))
                + coefs[7] * np.sin(t * 3 * 2 * np.pi / (365.25 * 24 * 60 * 60 * 1000))
    ...
    # getRegion() mixes a string "id" column with the numeric ones, so numpy stores the
    # whole table as strings - cast both columns we actually use.
    dates_obs = np.array(timeseries["time"], dtype="int64")
